import { ExecutionContext, ProjectStack } from '@architect-guardian/shared-types';
...
describe('Architectural Skills Verification', () => {
  ...
  const projectPath = path.resolve(process.cwd(), '../../test-fixtures/violations');
  ...
  it('should inject Atomic Design metadata into the prompt for Button.tsx', async () => {
    ...
    const skillPath = path.join(skillsDir, manifest.name);
    ...
    const result = await engine.executeSkill(manifest, context, skillPath);
