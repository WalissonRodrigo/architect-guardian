...
export class ArchitectGuardianChatParticipant {
    ...
    public register(context: vscode.ExtensionContext) {
        const participant = vscode.chat.createChatParticipant('architect-guardian', this.handler.bind(this));
    ...
    private async handler(
        ...
        if (!client) {
            responseStream.markdown('The Architect Guardian MCP Server is not running. Please start it using the command palette.');
            return;
        }
        ...
        try {
            ...
            const currentProjectPath = workspaceFolders && workspaceFolders.length > 0
...
2. After receiving a tool result, answer the user normally using markdown headers and bullet points.`;
            ...
            if (!model) {
                 responseStream.markdown('No suitable language model found. Please ensure GitHub Copilot or another Chat Provider is active.');
                 return;
            }
